<template>
    <Doc>
        ...
        <P>初始化MyDialog后，Vue原型上会增添一个$MyDialog属性，我们可以在Vue中通过使用<Strong>this.$MyDialog</Strong>访问到该对象。通过该对象的open方法可以创建对话框，创建时需要传入一个对话框配置项，这里介绍几个常用配置项，更多配置请参考、、、、、、、、。</P>
        <H3>content</H3>
        ...
        <H3>用vue文件做对话框内容</H3>
        <P>这是<Strong>my-vue-dialog</Strong>最推荐的方法，尤其对于创建非常复杂的对话框时候非常有用。使用vue文件定义控件(Vue的sfc控件)也是目前Vue最流行的定义控件方式。做对话框内容例子如下：</P>
        <Demo :demo="Open2" title="用vue文件做对话框内容" :code="open2Code">
            <P>使用component创建对话框，这里用一个vue文件做componentOptions。</P>
        </Demo>
        
        <H3>用class类及装饰器做对话框内容</H3>
        <P>Vue支持用官方的装饰器<A href="https://github.com/vuejs/vue-class-component">vue-class-component</A>及class语法创建控件，<Strong>my-vue-dialog</Strong>也支持这种方式创建对话框</P>
        ...
        <P>在Vue中，我们可以使用<Strong>Vue.extend</Strong>创建子类，<Strong>my-vue-dialog</Strong>也支持创建的子类做对话框内容。</P>
        ...
        <H2>$myDialog</H2>
        <P>被打开的对话框，会增添一个$myDialog属性，通过该属性关闭对话框、修改标题、修改对户框尺寸等功能。</P>
